export const contentMessages = {
  courses: {
    filters: {
      private: "Private (One to One)",
      record: "Record Course",
      live: "Live Course",
      free: "Free Course",
    },
    gender: {
      all: "All",
      children: "Children",
      male: "Male",
      female: "Women",
    },
    empty: {
      title: "No courses in this category yet",
      description:
        "We are expanding our {category} offerings soon. In the meantime, explore our Private or Free courses — or book a free trial and our team will recommend the best learning path for you.",
      cta: "Book a free trial class",
    },
    targets: {
      children: "Children",
      male: "Male",
      female: "Women",
    },
    items: {
      "noorani-qaida": {
        title: "Noorani Qaida Course",
        description: "Foundation Arabic alphabet and Quran reading basics",
      },
      "quran-hifz-male": {
        title: "Quran Hifz Course (Male)",
        description: "Complete Quran memorization for males",
      },
      "quran-hifz-female": {
        title: "Quran Hifz Course (Women)",
        description:
          "Complete Quran memorization for women with female teachers",
      },
      "tajweed-male": {
        title: "Quran Learning with Tajweed (Male)",
        description: "Proper Quran recitation with Tajweed rules",
      },
      "tajweed-female": {
        title: "Quran Learning with Tajweed (Women)",
        description: "Tajweed for women with female certified teachers",
      },
      "free-learning": {
        title: "Free Quran Learning Course",
        description: "Start your Quran journey at no cost",
      },
    },
  },
  pricing: {
    month: "Monthly",
    perMonth: "/month",
    usdMonthly: "{usd} USD / {month}",
    mostPopular: "Most Popular",
    getStarted: "Get Started — ৳{price}/mo",
    familyIntro:
      "We understand tuition fees are important for many families. At BD Online Quran Academy, we strive to keep quality Quran education affordable without compromising on teacher standards or personal care.",
    familyDetail:
      "A 20% discount on tuition fees will apply for the second child and all subsequent children enrolled in our private learning packages.",
    packages: {
      basic: {
        name: "Basic",
        features: [
          "One-to-one class benefits",
          "Classes 2 days a week",
          "8 days of classes per month",
          "Class duration: 30 minutes",
          "Convenient class times",
          "Noorani/Najera Basic",
        ],
      },
      standard: {
        name: "Standard",
        features: [
          "One-to-one class benefits",
          "Classes 3 days a week",
          "12 days of classes per month",
          "Class duration: 40 minutes",
          "Convenient class times",
          "Najera with Tajweed",
        ],
      },
      advance: {
        name: "Advance",
        features: [
          "One-to-one class benefits",
          "Classes 4 days a week",
          "16 days of classes per month",
          "Class duration: 50 minutes",
          "Convenient class times",
          "Najera with Tajweed",
        ],
      },
      premium: {
        name: "Premium",
        features: [
          "One-to-one class benefits",
          "Classes 5 days a week",
          "20 days of classes per month",
          "Class duration: 60 minutes",
          "Convenient class times",
          "Hifz/Advance Tajweed",
        ],
      },
    },
  },
  ebooks: {
    allBooks: "All Books",
    featuredBooks: "Featured Books",
    filterLabel: "E-book categories",
    featuredBadge: "Featured",
    pages: "pages",
    download: "Download",
    view: "View",
    viewAll: "View all",
    sortBy: "Sort by:",
    sortNewest: "Newest",
    sortTitle: "Title",
    sortPages: "Pages",
    viewToggle: "Book layout",
    viewGrid: "Grid view",
    viewList: "List view",
    emptyCategory: "No books in this category yet. Try another filter.",
    stackAlt: "Stack of Islamic books",
    reader: {
      eyebrow: "BDOQ Academy · E-book Reader",
      close: "Close reader",
      loading: "Loading book…",
      previousBook: "Previous book",
      nextBook: "Next book",
      fullPage: "Full page",
      share: "Share",
      open: "Open",
      reading: "Reading: {title}",
      languageBoth: "English & Bengali",
      languageBn: "Bengali",
      languageEn: "English",
    },
    detail: {
      backToAll: "Back to all e-books",
      author: "Author",
      pagesLabel: "Pages",
      size: "Size",
      language: "Language",
      downloadPdf: "Download PDF",
      fullscreenReader: "Full-screen reader",
      readOnline: "Read online",
      openNewTab: "Open in new tab",
      moreInCategory: "More in this category",
      coverAlt: "{title} cover",
      readTitle: "Read {title}",
    },
    categories: {
      tajweed: "Tajweed Books",
      islamicHistory: "Islamic History",
      religious: "Religious Books",
    },
    items: {
      "tajweed-made-easy": {
        title: "Tajweed Rules Made Easy",
        description:
          "A practical guide to essential Tajweed rules for correct Quran recitation, with examples for beginners.",
      },
      "tajweed-pronunciation": {
        title: "Correct Pronunciation in Quran Recitation",
        description:
          "Makharij and Sifaat explained step by step for students learning Tajweed with a teacher.",
      },
      "seerah-overview": {
        title: "Brief History of Islam — Seerah Overview",
        description:
          "Key events from the life of Prophet Muhammad (ﷺ) and the early Muslim community for young learners.",
      },
      "khulafa-rashidun": {
        title: "The Rightly Guided Caliphs",
        description:
          "An introduction to the Khulafa Rashidun and their contributions to Islamic civilization.",
      },
      "daily-duas": {
        title: "Daily Duas for Muslim Families",
        description:
          "Essential supplications for morning, evening, and daily activities with transliteration.",
      },
      "pillars-of-islam": {
        title: "Understanding the Five Pillars of Islam",
        description:
          "A clear explanation of Shahada, Salah, Zakat, Sawm, and Hajj for students and parents.",
      },
    },
  },
  audio: {
    nowPlaying: "Now Playing",
    progress: "Audio progress",
    play: "Play audio",
    pause: "Pause audio",
    previous: "Previous surah",
    next: "Next surah",
    repeat: "Repeat",
    download: "Download audio",
    volume: "Mute or unmute",
    surahListTitle: "Quran Surah List",
    surahListSubtitle: "Select a surah to listen",
    viewAllSurahs: "View All Surahs",
    otherRecitations: "Other Recitations",
    viewAll: "View all",
    featuredSubtitle: "{count} verses · Featured recitation",
  },
  videos: {
    featuredBadge: "Featured Video",
    playlist: "Playlist",
    viewAll: "View all",
    otherVideos: "Other Videos",
    filterLabel: "Video categories",
    sortBy: "Sort by:",
    sortLatest: "Latest",
    sortPopular: "Popular",
    sortTitle: "Title",
    loadMore: "Load More Videos",
    emptyCategory: "No videos in this category yet. Try another filter.",
    views: "{count} views",
    recent: "Recently added",
    categories: {
      allVideos: "All Videos",
      quranLearning: "Quran Learning",
      tajweed: "Tajweed",
      courses: "Courses",
      studentStories: "Student Stories",
    },
  },
  gallery: {
    tabs: {
      ariaLabel: "Gallery media categories",
      all: "All",
      video: "Video",
      audio: "Audio",
      gallery: "Gallery",
      image: "Image",
    },
    itemCount: "{count} items",
    empty: {
      all: {
        title: "No gallery items yet",
        description:
          "Photos, videos, and audio from BDOQ Academy classes and events will appear here.",
      },
      gallery: {
        title: "No gallery items yet",
        description:
          "Photos, videos, and audio from BDOQ Academy classes and events will appear here.",
      },
      image: {
        title: "No images yet",
        description:
          "Academy photos and class moments will be shown in this section.",
      },
      video: {
        title: "No videos yet",
        description:
          "Quran lessons, class highlights, and academy videos will appear here.",
      },
      audio: {
        title: "No audio yet",
        description:
          "Recitations and audio clips from BDOQ Academy will appear here.",
      },
    },
    collections: {
      onlineClass: "Online Class Sessions",
      nooraniQaida: "Noorani Qaida Classes",
      hifzProgram: "Hifz Program",
      eventsSeminars: "Events & Seminars",
      quranRecitation: "Quran Recitation",
      teacherTraining: "Teacher Training",
      studentActivities: "Student Activities",
      certificatesAwards: "Certificates & Awards",
      communityGathering: "Community Gathering",
      internationalStudents: "International Students",
    },
    albums: {
      all: "All",
      gallery01: "Gallery 01",
      gallery02: "Gallery 02",
      gallery03: "Gallery 03",
    },
    items: {
      g1: { title: "Online class session" },
      g2: { title: "Student recitation practice" },
      g3: { title: "Teacher mentoring session" },
      g4: { title: "Noorani Qaida class" },
      g5: { title: "Tajweed workshop" },
      g6: { title: "Hifz progress review" },
      g7: { title: "Female teachers team" },
      g8: { title: "International students" },
      g9: { title: "Academy orientation" },
      g10: { title: "Certificate ceremony" },
      g11: { title: "Parent meeting" },
      g12: { title: "Community gathering" },
    },
    lightbox: {
      close: "Close lightbox",
      previous: "Previous image",
      next: "Next image",
      of: "{current} of {total}",
    },
  },
  blog: {
    allTopics: "All Topics",
    readMore: "Read More",
    featuredArticles: "Featured Articles",
    moreArticles: "More Articles",
    viewAllArticles: "View All Articles",
    popularArticles: "Popular Articles",
    youMayAlsoLike: "You May Also Like",
    stayUpdated: "Stay Updated",
    newsletterDesc:
      "Subscribe to get the latest articles and tips on Quran learning delivered to your inbox.",
    newsletterPlaceholder: "Your email address",
    subscribe: "Subscribe",
    newsletterSuccess: "Thanks for subscribing!",
    newsletterError: "Could not subscribe. Please try again.",
    filterAria: "Filter articles by topic",
    articleQuote:
      "The best among you are those who learn the Quran and teach it.",
    articleQuoteSource: "Sahih al-Bukhari",
    academyCalloutTitle: "Learn with BDOQ Academy teachers",
    academyCalloutDesc:
      "Our certified Hafiz teachers guide children and adults through Tajweed, Hifz, and Quran reading — one-to-one, with flexible times.",
    academyCalloutCta: "Book a free trial class",
    shareStoryTitle: "Have a story or tip to share?",
    shareStoryDesc:
      "We welcome parent experiences, student journeys, and practical Quran learning tips for our blog.",
    shareStoryCta: "Share Your Story",
    noArticles: "No articles found",
    noArticlesDesc:
      "Try another keyword or clear the topic filter to see all posts.",
    emptyPostsTitle: "No blog posts yet",
    emptyPostsDesc:
      "We are preparing insightful articles about Quran education, Tajweed, and Islamic learning. Check back soon!",
    articleFallback: "Article",
    academyBlog: "BDOQ Academy Blog",
    linkCopied: "Link copied",
    copyLink: "Copy link",
    searchAria: "Search blog posts",
    minRead: "{minutes} min read",
    onThisPage: "On this page",
    shareArticle: "Share article",
    copied: "Copied",
    copy: "Copy",
    relatedArticles: "Related articles",
    startLearning: "Start learning today",
    previous: "Previous",
    next: "Next",
    home: "Home",
    blog: "Blog",
    contributor: "BDOQ Academy Contributor",
    contributorBio:
      "Sharing practical guidance on Quran learning, Tajweed, and online education from BD Online Quran Academy.",
    startLearningDesc:
      "Book a free trial class with a certified Hafiz teacher — one-to-one, flexible times.",
    returnHome: "Return to Home",
    posts: {
      "importance-of-tajweed-in-quran-recitation": {
        title: "The Importance of Tajweed in Quran Recitation",
        excerpt:
          "Learn why Tajweed is essential for every Muslim and how BDOQ Academy helps students master correct pronunciation.",
        content: `Tajweed is the set of rules governing the correct pronunciation of the Quran. Allah (SWT) revealed the Quran in clear Arabic, and reciting it with proper Tajweed preserves the meaning and beauty of every word.

At BD Online Quran Academy, students learn Tajweed through one-to-one live classes with qualified teachers. Whether you are a beginner or improving your recitation, personalized feedback helps you correct mistakes early.

**Key benefits of learning Tajweed:**
- Preserve the meaning of Quranic verses
- Recite with confidence in Salah and daily worship
- Build a strong foundation for Hifz (memorization)
- Follow the Sunnah of reciting the Quran beautifully

BDOQ Academy offers separate Tajweed courses for men and women, with flexible scheduling for students in Bangladesh and abroad. Book a free trial class to experience our teaching method firsthand.`,
        tags: ["Tajweed", "Quran", "Learning"],
      },
      "how-to-start-online-quran-learning": {
        title: "How to Start Online Quran Learning — A Complete Guide",
        excerpt:
          "A step-by-step guide for parents and students beginning their Quran journey with BDOQ Academy's one-to-one system.",
        content: `Starting your Quran learning journey online is easier than ever with BD Online Quran Academy. Our structured approach ensures every student — child or adult — receives personal attention from an experienced teacher.

**Step 1: Choose your course**
Select from Noorani Qaida, Tajweed, Hifz, or our free learning course based on your current level and goals.

**Step 2: Book a free trial**
Register for a free trial class on our website. Share your preferred time, timezone, and whether you need a male or female teacher.

**Step 3: Meet your teacher**
During the trial, your assigned teacher assesses your level and recommends a learning plan and monthly package.

**Step 4: Begin regular classes**
Attend live one-to-one sessions from home with a device and stable internet. BDOQ teachers use reliable platforms optimized for Quran instruction.

With 7+ years of experience and 300+ students worldwide, BDOQ Academy is a trusted choice for online Quran education.`,
        tags: ["Guide", "Online Learning", "Students"],
      },
      "benefits-of-one-to-one-quran-classes": {
        title: "5 Benefits of One-to-One Quran Classes",
        excerpt:
          "Discover why personalized Quran instruction delivers faster progress than group classes for most learners.",
        content: `One-to-one Quran classes are at the heart of BD Online Quran Academy's teaching model. Here are five reasons students thrive in this format:

**1. Personalized pace**
Your teacher adjusts each lesson to your speed — no rushing ahead or waiting for others.

**2. Immediate correction**
Tajweed mistakes are corrected in real time, building accurate habits from day one.

**3. Flexible scheduling**
Choose class times that fit your work, school, or family routine across any timezone.

**4. Privacy for sisters**
Female students learn exclusively with qualified female teachers in a comfortable environment.

**5. Monthly progress tracking**
Teachers provide regular check-ups, assignments, and analytics so you see measurable improvement.

BDOQ Academy packages start from ৳2,000/month with classes 2–5 days per week. Contact us or book a free trial to get started.`,
        tags: ["One-to-One", "Benefits", "BDOQ Academy"],
      },
    },
  },
  about: {
    purposeEyebrow: "Purpose & Direction",
    purposeTitle: "Mission & Vision — BDOQ Academy",
    purposeSubtitle:
      "Our purpose and future goals — combining technology, quality, and Islamic values.",
    mission: "Our Mission",
    missionSubtitle:
      "Delivering quality, safe, and structured Quran education for every learner.",
    vision: "Our Vision",
    visionSubtitle:
      "Becoming a globally trusted online Quran education platform.",
    classSystemTitle: "BDOQ Academy — Class System & Promise",
    classSystem: "Class system",
    ourPromise: "Our promise",
    promiseText:
      "We promise every student a dedicated teacher, transparent progress updates, flexible scheduling, and a safe learning environment — whether you study Tajweed, Hifz, or Noorani Qaida with BDOQ Academy.",
    missionPoints: [
      "Provide one-to-one and structured Quran education for all ages",
      "Connect students with qualified Hafiz, scholars, and certified teachers",
      "Offer flexible online classes for Bangladesh and international students",
      "Maintain separate, privacy-focused classes for men, women, and children",
      "Deliver multilingual instruction in Bengali, English, Arabic, and Urdu",
      "Support families with affordable packages and transparent progress tracking",
    ],
    visionPoints: [
      "Become a globally trusted name in online Quran education",
      "Empower 10,000+ students to read, understand, and memorize the Quran",
      "Train and onboard dedicated teachers from Bangladesh and abroad",
      "Use technology to make Islamic learning engaging and effective",
      "Build a community rooted in sincerity, discipline, and Islamic values",
      "Expand record, live, and private learning pathways for every learner",
    ],
    featuredVideoTitle: "Welcome to BD Online Quran Academy",
    featuredVideoDesc:
      "Discover how BDOQ Academy delivers one-to-one online Quran education with experienced teachers worldwide.",
  },
} as const;
