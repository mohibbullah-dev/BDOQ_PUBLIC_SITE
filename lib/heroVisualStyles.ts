...
export function heroPanelSoft(className?: string): string {
  return cn(
    "rounded-2xl border border-[#1B6B44]/20 bg-[#F0FBF6] p-4 shadow-[0_4px_16px_rgba(27,107,68,0.08)]",
    className
  );
}
...
export function heroBadge(className?: string): string {
  return cn(
    "inline-flex items-center rounded-full border border-[#1B6B44]/25 bg-white px-3 py-1",
    "font-inter text-[11px] font-bold uppercase tracking-wider text-[#0D4A2F]",
    "shadow-sm",
    className
  );
}
